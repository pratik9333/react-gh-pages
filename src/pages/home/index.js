...
const Home = () => {
  ...
  useEffect(() => {
    // note - data was prefetch for testing.. please uncomment the function to fetch data
    // and remove the extra loading below function :)
          ...
          <div className="container">
            <div className="main">
              {Array.isArray(Data) && Data.length > 0 ? (
                Data?.map((card) => (
                  <div key={card.id} className={card.design_type + "-main"}>
                    {switchCase(card.design_type, card.cards)}
                  </div>
                ))
              ) : (
                <center>
                  <h2>{Data}, Please try again later!</h2>
                </center>
              )}
            </div>
          </div>
